...
ccrec() {
    ...
    local OFX="${HOME}/path/to/export-${STMT_DATE}.qfx"

    if [[ "${STMT_DATE}" =~ ^(19|20)[0-9]{2}(0[1-9]|1[0-2])26$ ]]; then
        source "${PROJ}/.venv/bin/activate"
        ccct --credential-dir=CHANGEME --ofx-file="${OFX}" --bank-id=CHANGEME --statement-date="${STMT_DATE}" --alloc-columns='CHANGEME' --document-id='CHANGEME'
        deactivate
    else
...
ccrec() {
    ...
    source "${PROJ}/.venv/bin/activate"
    if [[ "${STMT_DATE}" =~ ^(19|20)[0-9]{2}(0[1-9]|1[0-2])26$ ]]; then
        ccct --ofx-file="${OFX}" --statement-date="${STMT_DATE}"
    else
        echo "Error: Invalid statement date!"
    fi
    deactivate
...
### Python3 Stuff
CCCT is packaged as a normal Python project. The runtime dependencies, Python
version requirement, package metadata, and `ccct` command are declared in
`pyproject.toml`. You should not need to install the runtime libraries one at a
time.

For development, the simplest path is [`uv`](https://docs.astral.sh/uv/):
```
uv sync
uv run ccct --help
uv run pytest
...
`uv sync` creates or updates the local `.venv` from `pyproject.toml` and
`uv.lock`. `uv run` runs a command inside that environment, so you do not have
to manually activate it.

If you prefer to activate the environment yourself after `uv sync`:
```
...
If you are not using `uv`, any standard Python packaging tool can install the
project. For example:
```
python3 -m venv .venv
source .venv/bin/activate
...
Tests use `pytest`, which is a development dependency rather than a runtime
dependency. With `uv`, it is installed by the default development environment.
With other tools, install the `dev` dependency group if supported, or install
`pytest` directly in your development environment.
