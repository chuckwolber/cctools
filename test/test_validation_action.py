    ...
    def test__is_valid_credential_dir(self):
        for i in const.INVALID_CREDENTIAL_DIRS:
            with self.subTest(i=i):
                self.assertRaises(argparse.ArgumentTypeError, CCConsoleArgs.is_valid_credential_dir, i)
        for i in const.VALID_CREDENTIAL_DIRS:
            with self.subTest(i=i):
                self.assertTrue(os.path.exists(CCConsoleArgs.is_valid_credential_dir(i)))

    def test__is_valid_bank_id(self):
        for i in const.INVALID_BANK_ID_STRINGS:
            with self.subTest(i=i):
                self.assertRaises(argparse.ArgumentTypeError, CCConsoleArgs.is_valid_bank_id, i)
        for i in const.INVALID_BANK_ID_NUMBERS:
            with self.subTest(i=i):
                self.assertRaises(argparse.ArgumentTypeError, CCConsoleArgs.is_valid_bank_id, i)
        for i in const.VALID_BANK_IDS:
            with self.subTest(i=i):
                self.assertTrue(CCConsoleArgs.is_valid_bank_id(i) == i)

    def test__set_alloc_columns(self):
        for i in const.INVALID_ALLOC_COLUMNS:
            with self.subTest(i=i):
                self.assertRaises(argparse.ArgumentTypeError, CCConsoleArgs.set_alloc_columns, i)
        for i in const.VALID_ALLOC_COLUMNS:
            with self.subTest(i=i):
                self.assertTrue(CCConsoleArgs.set_alloc_columns(i) == i.split(":"))

    def test__is_valid_ofx_file(self):
        for i in const.MISSING_OFX_FILES:
            with self.subTest(i=i):
                self.assertRaises(argparse.ArgumentTypeError, CCConsoleArgs.is_valid_ofx_file, i)
        for i in const.VALID_OFX_FILES:
            with self.subTest(i=i):
                self.assertTrue(CCConsoleArgs.is_valid_ofx_file(i))

    def test__is_valid_statement_date(self):
        for i in const.INVALID_STATEMENT_DATE_STRINGS:
            with self.subTest(i=i):
                self.assertRaises(argparse.ArgumentTypeError, CCConsoleArgs.is_valid_statement_date, i)
        for i in const.INVALID_STATEMENT_DATE_VALUES:
            with self.subTest(i=i):
                self.assertRaises(ValueError, CCConsoleArgs.is_valid_statement_date, i)
        for i in const.VALID_STATEMENT_DATES:
            with self.subTest(i=i):
                self.assertTrue(CCConsoleArgs.is_valid_statement_date(i))

    def test__is_valid_config_file(self):
        for i in const.MISSING_CONFIG_FILES:
            with self.subTest(i=i):
                self.assertRaises(argparse.ArgumentTypeError, CCConsoleArgs.is_valid_config_file, i)
        for i in const.INVALID_CONFIG_FILES:
            with self.subTest(i=i):
                self.assertRaises(argparse.ArgumentTypeError, CCConsoleArgs.is_valid_config_file, i)
        with self.subTest():
            self.assertIsInstance(CCConsoleArgs.is_valid_config_file(const.VALID_CONFIG), dict)
        for i in const.VALID_SCHEMA_FILES:
            with self.subTest(i=i):
                config = CCConsoleArgs.is_valid_config_file(const.VALID_CONFIG, i)
